				...
				if (GetDistanceBetweenCoords(coords, v.Deleter.Pos.x, v.Deleter.Pos.y, v.Deleter.Pos.z, true) < v.Deleter.Size.x) then
					if IsPedInAnyVehicle(playerPed) then
						...
						if DoesEntityExist(vehicle) then
							isInMarker = true
							vehicleDeleter = true
							CurrentActionData = {airport = k, vehicle = vehicle}
						end
					end
				end
			end
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "a_m_y_business_02",-1343.895, -2690.148, 13.000, true, true)
    SetBlockingOfNonTemporaryEvents(ped, true)
    FreezeEntityPosition(ped, true)
end)

local v1 = vector4(-1343.895, -2690.148, 13.000, 8.000)
...
Citizen.CreateThread(function()
    while true do
        ...
        if Vdist2(GetEntityCoords(PlayerPedId(), false), v1) < distance then
            Draw3DText(v1.x,v1.y,v1.z, "Prêt a prendre du service ?")
        end
    end
end)